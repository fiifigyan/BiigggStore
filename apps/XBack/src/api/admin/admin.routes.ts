...
router.post('/upload', upload.array('images'), async (req: any, res, next) => {
  try {
    ...
    const uploadOne = (file: any) => new Promise((resolve, reject) => {
      const stream = cloudinary.uploader.upload_stream({ folder: 'xstore' }, (error: any, result: any) => {
    ...
    const results = await Promise.all((files as any[]).map((f) => uploadOne(f)));
    const urls = results.map((r: any) => r.secure_url || r.url).filter(Boolean);
...
router.post('/upload/sign', async (_req: any, res, next) => {
  try {
...
router.post('/upload/delete', async (req: any, res, next) => {
  try {
...
router.put('/products/:id', async (req, res, next) => {
  try {
    ...
    const deletedImages = existing?.images?.filter((url: string) => !newImages.includes(url)) || [];

    if (deletedImages.length) {
      await Promise.all(deletedImages.map((url: string) => deleteImageFromCloudinary(url)));
    }
...
router.delete('/products/:id', async (req, res, next) => {
  try {
    ...
    if (existing?.images?.length) {
      await Promise.all(existing.images.map((url: string) => deleteImageFromCloudinary(url)));
    }
...
router.put('/orders/:id', async (req, res, next) => {
  try {
    ...
    if (order) {
      notifier.emit('notification', { type: 'order:update', message: `Order ${order.orderNumber} status updated to ${order.status}`, data: { orderId: order.id } });
    }
